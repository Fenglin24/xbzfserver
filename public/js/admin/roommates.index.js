var adminIndex = {
    ...
    initDeleteBtn: function(self) {
        $('.delete').click(function() {
            ...
            pop.confirm('确认删除？不可恢复！', function() {
                http.ajax(baseURL + '?s=admin/roommates/delete', data, function(res) {
                    if (res.code == 0) {
                        trDom.remove();
                        pop.closeAll();
                    }
    ...
    inttxxBtn: function(self) {
    	$('.xx').click(function() {
            ...
            pop.confirm('确认'+buttonText, function() {
                http.ajax(baseURL + '?s=admin/roommates/xx', data, function(res) {
                    if (res.code == 0) {
                        // trDom.remove();
                        location.reload();
                        // pop.closeAll();
    ...
    initDeleteAllBtn: function(self) {
    	$('.deleteall').click(function() {
            ...
            var data = { id: ids };
            pop.confirm('确认删除？不可恢复！', function() {
                http.ajax(baseURL + '?s=admin/roommates/deleteAll', data, function(res) {
                    if (res.code == 0) {
                    location.reload();
                    }
                });
    ...
    initXxAllbtn:function(self) {
    	$('.xxall').click(function() {
            ...
            pop.confirm('确认下线？不可恢复！', function() {
                http.ajax(baseURL + '?s=admin/roommates/xxAll', data, function(res) {
                    if (res.code == 0) {
                    location.reload();
                    }
                });
    ...
    initLhBtn: function(self, key) {
        $('.'+key).click(function() {
            ...
            http.ajax(baseURL + '?s=admin/roommates/update_tag', data, function(res) {
                if (res.code == 0) {
                    // trDom.remove();
