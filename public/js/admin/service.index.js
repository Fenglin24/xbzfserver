var adminIndex = {
	...
	initEditDialog: function(self) {
		...
		$('#editFormPage #submit').click(function() {
			...
			http.ajax(baseURL + '?s=admin/service/' + action, data, function(res) {
				if (res.code == 0) {
	...
	initDeleteBtn: function(self) {
		$('.delete').click(function(){
			...
			pop.confirm('确认删除？不可恢复！', function(){
				http.ajax(baseURL + '?s=admin/service/delete', data, function(res) {
					if (res.code == 0) {
						trDom.remove();
						pop.closeAll();
					}
	...
	initLhBtn: function(self) {
		$('.lh').click(function(){
			...
			pop.confirm('确认拉黑？', function(){
				http.ajax(baseURL + '?s=admin/service/lh', data, function(res) {
					if (res.code == 0) {
						// trDom.remove();
						location.reload();
					}
