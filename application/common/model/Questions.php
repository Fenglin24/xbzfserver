...
class Questions extends Model {
    ...
    public function deleteQuestionsData($id) {
	...
	public function saveQuestionsData($data) {
		...
		if (!isset($data['id'])) {
			// if (empty($data['Questionsname'])) {
			// 	return ['code' => -1, 'msg' => '请提供用户名'];
			// }
			// // if (empty($data['password'])) {
			// 	return ['code' => -1, 'msg' => '请提供密码'];
			// }
			// if (self::get(['Questionsname' => $data['Questionsname']])) {
			...
			// if (self::get(['Questionsname' => $data['Questionsname'], 'id' => ['neq', $data['id']]])) {
			// 	return ['code' => -1, 'msg' => '用户名已存在'];
			// }
